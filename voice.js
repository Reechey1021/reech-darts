...
// Gate voice listening when scoring is not allowed (e.g., online game with mutual control OFF and it is not your turn).
let voiceTurnAllowed = true;
let _turnOverlayObserver = null;
...
function computeVoiceTurnAllowedFromOverlay() {
  const state = app?.latestState;
  const isOnline = state?.match?.gameType === "online";
  const mutualOff = isOnline && state?.match && !state.match.allowMutualControl;

  // Only gate in the specific scenario requested: online + mutual control OFF.
  if (!mutualOff) return true;

  const overlay = document.getElementById("turnOverlay");
  if (!overlay) return true;

  // Overlay visible => "It's other player's turn" (scoring not allowed). Stop listening.
  const overlayVisible = !overlay.classList.contains("hidden");
  return !overlayVisible;
}

function syncVoiceToTurnOverlayGate() {
  voiceTurnAllowed = computeVoiceTurnAllowedFromOverlay();

  // If voice mode isn't active, don't force anything.
  if (app.inputMode !== "voice") return;

  if (!voiceTurnAllowed) {
    // Stop listening immediately to avoid background score submission glitches.
    stopVoiceAuto();
    setVoiceStatus("Waiting for your turn…");
    return;
  }

  // If it's now allowed, resume the always-on behavior.
  startVoiceAuto();
}

function setupTurnOverlayObserver() {
  if (_turnOverlayObserver) return;
  const overlay = document.getElementById("turnOverlay");
  if (!overlay) return;

  _turnOverlayObserver = new MutationObserver(() => {
    try { syncVoiceToTurnOverlayGate(); } catch (_) {}
  });

  try {
    _turnOverlayObserver.observe(overlay, { attributes: true, attributeFilter: ["class"] });
  } catch (_) {
    _turnOverlayObserver = null;
  }

  // Initial sync
  syncVoiceToTurnOverlayGate();
}
...
export function initVoiceUI() {
  ...
  btn.addEventListener("click", async () => {
    if (app.inputMode !== "voice") return;
    // If scoring is not allowed (e.g., online mutual control OFF and not your turn), block voice capture.
    if (!voiceTurnAllowed) {
      setVoiceStatus("Waiting for your turn…");
      return;
    }
