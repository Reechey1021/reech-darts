...
// ------------------------------
// Setup modal UI helpers (steppers + button rows)
// Mirrors the setupModal logic used on /game/ (app/ui/events.js), but scoped to Arcade setup modals.
const __arcadeSetupUI = {
  steppers: new Set(),
  groups: new Set(),
  toggles: new Set(),
};

function arcadeSyncStepperFromControl(targetId) {
...
function arcadeSyncGroupFromControl(targetId) {
  const ctrl = document.getElementById(targetId);
  if (!ctrl) return;
...
function arcadeSyncToggleFromControl(targetId) {
  const ctrl = document.getElementById(targetId);
...
function initArcadeSetupUIControls() {
  // Steppers
  const steppers = Array.from(document.querySelectorAll("[data-setup-stepper][data-target]"));
  for (const stepper of steppers) {
    ...
    __arcadeSetupUI.steppers.add(targetId);
    ...
    const setByIndex = (i) => {
      ...
      arcadeSyncStepperFromControl(targetId);
    };
    ...
    ctrl.addEventListener("change", () => arcadeSyncStepperFromControl(targetId));
    arcadeSyncStepperFromControl(targetId);
  }

  // Button groups
  const groupRows = Array.from(document.querySelectorAll(".setupBtnRow[data-setup-group][data-target]"));
  for (const row of groupRows) {
    ...
    __arcadeSetupUI.groups.add(targetId);
    ...
    for (const b of btns) {
      b.addEventListener("click", () => {
        ...
        ctrl.value = v;
        ctrl.dispatchEvent(new Event("change", { bubbles: true }));
        arcadeSyncGroupFromControl(targetId);
      });
    }
    arcadeSyncGroupFromControl(targetId);
  }
  ...
  // Toggles
  const toggleRows = Array.from(document.querySelectorAll(".setupBtnRow[data-setup-toggle][data-target]"));
  for (const row of toggleRows) {
    ...
    __arcadeSetupUI.toggles.add(targetId);
    ...
    for (const b of btns) {
      b.addEventListener("click", () => {
        ...
        ctrl.checked = (v === "on");
        ctrl.dispatchEvent(new Event("change", { bubbles: true }));
        arcadeSyncToggleFromControl(targetId);
      });
    }

    ctrl.addEventListener("change", () => arcadeSyncToggleFromControl(targetId));
    arcadeSyncToggleFromControl(targetId);
  }
}

function arcadeSyncAllSetupUIControls() {
  __arcadeSetupUI.steppers.forEach((id) => arcadeSyncStepperFromControl(id));
  __arcadeSetupUI.groups.forEach((id) => arcadeSyncGroupFromControl(id));
  __arcadeSetupUI.toggles.forEach((id) => arcadeSyncToggleFromControl(id));
}
...
function setAvatarImg(imgEl, url) {
  ...
  const prev = imgEl.dataset.lastSrc || "";
  if (!next) {
    imgEl.dataset.lastSrc = "";
    // Keep element present but hidden so flex layout remains stable.
...
function renderReadyRoom(state) {
  ...
  // Seat2 placeholder (invite/copy) is rendered to be visually identical to the player card.
  // We keep the legacy invite/copy IDs in the DOM for wiring, but hide them and render a proxy UI.
  let showInviteProxy = false;
  try {
    const tools = qs("readySeat2EmptyTools");
    const everFilled = !!(state?.match?.arcade?.seat2EverFilled);
    showInviteProxy = !!(iAmHost && !seat2 && !everFilled);
  ...
  const cardHtml = ({ title, name, photoURL, ready, avatarHtml, badgeHtml }) => {
    const img = photoURL
      ? `<img class="dashAvatar" src="${photoURL}" alt="Player photo" />`
      : `<div class="dashAvatar hidden"></div>`;
    const avatar = avatarHtml || img;
    const badgeBlock = (typeof badgeHtml === "string") ? badgeHtml : badge(ready);
    return `
      <div class="readyroomdashIdentity" style="margin-bottom:14px; justify-content:space-between;">
        <div class="readyDIwrapper">
          ...
          <div style="display: flex; flex-direction: column; align-items: center; margin-top: 10px;">
            <div class="dashName readyroom">${name || "—"}</div>
          </div>
        </div>
        <div>${badgeBlock}</div>
      </div>
  ...
  if (p2Card) {
    if (seat2) {
      ...
      // Seat2 placeholder: avatar circle becomes a full click target for inviting friends.
      const plusAvatar = `
      ...
      `;
      const action = showInviteProxy
        ? `<span class="NotReady readyRoomInviteAction" role="button" tabindex="0" data-role="readyCopyInviteProxy">Copy Link</span>`
        : `<span class="NotReady" style="opacity:.6;">WAITING</span>`;
      p2Card.innerHTML = cardHtml({
        title: "INVITE",
        name: "Invite Friends",
        photoURL: null,
        ready: false,
        avatarHtml: plusAvatar,
        badgeHtml: action,
      });
    }
  }
...
async function maybeAutoStartFromReady(ref, state) {
  ...
  await app.db.runTransaction(async (tx) => {
    ...
    // Resolve starter for online games at the moment the match begins
    if (fresh.match.arcade.starterSeat !== 0 && fresh.match.arcade.starterSeat !== 1) {
      fresh.match.arcade.starterSeat = resolveStarterSeat(fresh.match.arcade.starter);
    }

    // Seed currentPlayer from the resolved starter (only if no history yet)
    const starterSeat = (fresh.match.arcade.starterSeat === 1) ? 1 : 0;
    const mode = String(fresh.match.arcade.mode || "bull_challenge");
    if (mode === "bull_challenge" && fresh.match.arcade.bcState && Array.isArray(fresh.match.arcade.bcState.history) && fresh.match.arcade.bcState.history.length === 0) {
      fresh.match.arcade.bcState.currentPlayer = starterSeat;
    }
    if (mode === "around_the_clock" && fresh.match.arcade.atcState && Array.isArray(fresh.match.arcade.atcState.history) && fresh.match.arcade.atcState.history.length === 0) {
      fresh.match.arcade.atcState.currentPlayer = starterSeat;
    }
    if (mode === "high_score" && fresh.match.arcade.highScoreState && Array.isArray(fresh.match.arcade.highScoreState.history) && fresh.match.arcade.highScoreState.history.length === 0) {
      fresh.match.arcade.highScoreState.currentPlayer = starterSeat;
    }
    if (mode === "rounds" && fresh.match.arcade.roundsState && Array.isArray(fresh.match.arcade.roundsState.history) && fresh.match.arcade.roundsState.history.length === 0) {
      fresh.match.arcade.roundsState.currentPlayer = starterSeat;
    }
    if (mode === "race" && fresh.match.arcade.raceState && Array.isArray(fresh.match.arcade.raceState.history) && fresh.match.arcade.raceState.history.length === 0) {
      fresh.match.arcade.raceState.currentPlayer = starterSeat;
    }
...
const starterSeat = (Number(state?.match?.arcade?.starterSeat) === 1) ? 1 : 0;
const suddenDeath = state?.match?.arcade?.suddenDeath === true;

const baseMaxVisits = Math.max(0, rounds) * 2;
const atOrBeyondBase = (baseMaxVisits > 0) ? (totalVisits >= baseMaxVisits) : false;

// We only ever finish at the end of a round (after both players have thrown),
// so the result is fair regardless of starter.
const endOfRound = (totalVisits % 2) === 0;

let finished = false;
let winner = null;

if (atOrBeyondBase && endOfRound) {
  if (players[0].total > players[1].total) { finished = true; winner = 0; }
  else if (players[1].total > players[0].total) { finished = true; winner = 1; }
  else {
    // Draw after base rounds
    if (!suddenDeath) { finished = true; winner = null; }
    else { finished = false; winner = null; } // keep playing extra rounds until totals differ
  }
}

const roundIndex = Math.floor(totalVisits / 2);
const currentPlayer = finished ? 0 : ((starterSeat + totalVisits) % 2);
...
const endOfRound = (hist.length % 2) === 0;

let winner = null;
let finished = false;

// Competitive fairness: only decide the winner after both players have completed the same number of visits.
if (endOfRound) {
  const p0Hit = players[0].total >= target;
  const p1Hit = players[1].total >= target;

  if (p0Hit || p1Hit) {
    if (players[0].total > players[1].total) { winner = 0; finished = true; }
    else if (players[1].total > players[0].total) { winner = 1; finished = true; }
    else {
      // Perfect tie after a completed round
      if (!suddenDeath) { winner = null; finished = true; }
      else { winner = null; finished = false; }
    }
  }
}
...
function renderHighScore(state) {
  ...
  // Score modes use hsInputArea keypad/dartpad; hide other arcade keypads.
  const bcKeypadEl = qs("arcadeKeypad");     // Bull keypad (Outer/Bull/Miss)
  const atcKeypadEl = qs("atcKeypad");       // ATC keypad (S/D/T/Miss)
  if (bcKeypadEl) bcKeypadEl.classList.add("hidden");
  ...
  setAvatarImg(p1PhotoEl, p1Photo);
  setAvatarImg(p2PhotoEl, p2Photo);
...
function renderRounds(state) {
  ...
  setAvatarImg(p1PhotoEl, p1PhotoURL);
  setAvatarImg(p2PhotoEl, p2PhotoURL);
...
async function ensureDocHasAtcState(ref) {
  ...
  await app.db.runTransaction(async (tx) => {
    ...
    const freshAtc = computeAtcStateFromHistory({ startOn: startOnNum, direction, multipliers, starterSeat: state?.match?.arcade?.starterSeat, suddenDeath: state?.match?.arcade?.suddenDeath, exitType: exitType2, punishment: punishment2 }, []);
    ...
    if (winAchieved) {
      if (exitMode === "outer_and_bull") {
        if (pl.inExit && pl.exitArmed) {
          if (finishedAtVisit[p] === null) finishedAtVisit[p] = visitsTaken[p];
        }
      } else {
        if (pl.inExit) {
          if (finishedAtVisit[p] === null) finishedAtVisit[p] = visitsTaken[p];
...
// Competitive fairness: only declare a winner once BOTH players have had the same number of visits
// in the round where the first finish occurred.
const endOfRound = (visitsTaken[0] === visitsTaken[1]);
const p0Done = finishedAtVisit[0] !== null;
const p1Done = finishedAtVisit[1] !== null;

if (endOfRound) {
  if (p0Done && p1Done) {
    if (finishedAtVisit[0] === finishedAtVisit[1]) {
      // Both finished in the same number of visits (same round) => tie.
      finished = true;
      winner = null;
    } else {
      finished = true;
      winner = (finishedAtVisit[0] < finishedAtVisit[1]) ? 0 : 1;
    }
  } else if (p0Done || p1Done) {
    const w = p0Done ? 0 : 1;
    const wVisit = finishedAtVisit[w];
    // Only end once the opponent has had the same number of visits as the winner had when they finished.
    if (visitsTaken[1 - w] >= wVisit) {
      finished = true;
      winner = w;
    }
  }
...
async function submitAtcVisit(ref) {
  ...
  await app.db.runTransaction(async (tx) => {
    ...
    const multipliers = (atcCfg.multipliers !== false);
    const atc = computeAtcStateFromHistory({ startOn: startOnNum, direction, multipliers, starterSeat: state?.match?.arcade?.starterSeat, suddenDeath: state?.match?.arcade?.suddenDeath, exitType: ((atcCfg.exitType === "outer_and_bull" || atcCfg.exitType === "outer_or_bull") ? atcCfg.exitType : "bull"), punishment: ((atcCfg.punishment === 1 || atcCfg.punishment === 2 || atcCfg.punishment === 3) ? atcCfg.punishment : 0) }, atcRaw.history);
...
async function undoAtcVisit(ref) {
  ...
  await app.db.runTransaction(async (tx) => {
    ...
    hist.pop();
    const nextAtc = computeAtcStateFromHistory({ startOn: startOnNum, direction, multipliers, starterSeat: state?.match?.arcade?.starterSeat, suddenDeath: state?.match?.arcade?.suddenDeath, exitType: ((atcCfg.exitType === "outer_and_bull" || atcCfg.exitType === "outer_or_bull") ? atcCfg.exitType : "bull"), punishment: ((atcCfg.punishment === 1 || atcCfg.punishment === 2 || atcCfg.punishment === 3) ? atcCfg.punishment : 0) }, hist);
    tx.update(ref, { "match.arcade.atcState": nextAtc, updatedAt: new Date() });
  });
...
async function restartMatch(ref) {
  ...
  // If an end-of-game modal is open, ensure it closes on restart paths
  // (settings menu restart, modal restart button, etc.).
  try { closeHiddenModal(qs("arcadeEndModal")); } catch (_) {}
}
...
async function main() {
  ...
  // Arcade setup modals: steppers + toggle/button rows
  try { initArcadeSetupUIControls(); } catch (_) {}
...
// Proxy click target rendered inside the Seat2 placeholder card.
// This keeps the UI identical to the player card while reusing the same copy-link logic.
try {
  const rr = qs("readyRoomModal");
  if (rr) {
    const flashCopied = (el) => {
      try {
        if (!el) return;
        const orig = (el.dataset && el.dataset.origText) ? el.dataset.origText : (el.textContent || "Copy Link");
        if (el.dataset && !el.dataset.origText) el.dataset.origText = orig;
        el.textContent = "Copied";
        if (el.__copiedTimer) clearTimeout(el.__copiedTimer);
        el.__copiedTimer = setTimeout(() => {
          try { el.textContent = (el.dataset && el.dataset.origText) ? el.dataset.origText : "Copy Link"; } catch (_) {}
        }, 1200);
      } catch (_) {}
    };
    rr.addEventListener("click", (e) => {
      const invite = e.target && e.target.closest ? e.target.closest('[data-role="readyInviteFriendsProxy"]') : null;
      if (invite) {
        e.preventDefault();
        if (readyInviteFriendsBtn) readyInviteFriendsBtn.click();
        return;
      }
      const t = e.target && e.target.closest ? e.target.closest('[data-role="readyCopyInviteProxy"]') : null;
      if (!t) return;
      e.preventDefault();
      flashCopied(t);
      if (readyCopyBtn) readyCopyBtn.click();
    });
    rr.addEventListener("keydown", (e) => {
      const invite = e.target && e.target.closest ? e.target.closest('[data-role="readyInviteFriendsProxy"]') : null;
      if (invite && (e.key === "Enter" || e.key === " ")) {
        e.preventDefault();
        if (readyInviteFriendsBtn) readyInviteFriendsBtn.click();
        return;
      }
...
if (readyInviteFriendsBtn) readyInviteFriendsBtn.addEventListener("click", async () => {
  ...
  const openSetupForCurrentLobby = () => {
    ...
    const localBullFields = qs("bullSetupFieldsLocal");
    const onlineBullFields = qs("bullSetupFieldsOnline");
    ...
    if (localBullFields) localBullFields.classList.toggle("hidden", mode !== "bull_challenge");
    if (onlineBullFields) onlineBullFields.classList.toggle("hidden", mode !== "bull_challenge");
    ...
    const setNumberRowVisible = (inputId, visible) => {
      ...
      const stepper = document.querySelector(`[data-setup-stepper][data-target="${inputId}"]`);
      if (stepper) stepper.style.display = visible ? "" : "none";
    };

    const setToggleRowVisible = (targetId, visible) => {
      const row = document.querySelector(`.setupBtnRow[data-setup-toggle][data-target="${targetId}"]`);
      if (row) row.style.display = visible ? "" : "none";
      const maybeLabel = row ? row.previousElementSibling : null;
      if (maybeLabel && maybeLabel.classList && maybeLabel.classList.contains("setupLabel")) {
        maybeLabel.style.display = visible ? "" : "none";
      }
    };
    if (online) {
...
// Common competitive settings (starter / sudden death / mutual)
const starterChoice = String(st?.match?.arcade?.starter || "p1");
const suddenDeath = st?.match?.arcade?.suddenDeath === true;
const mutualControl = st?.match?.allowMutualControl !== false;

// Starter selects (safe to set even if hidden for other modes)
const starterIds = ["bullOnlineStarterSelect","hsOnlineStarterSelect","roundsOnlineStarterSelect","raceOnlineStarterSelect","atcOnlineStarterSelect"];
for (const sid of starterIds) {
  const el = qs(sid);
  if (el) el.value = starterChoice;
}

// Sudden death toggles
const sdIds = ["arcadeOnlineSuddenDeathChk","hsOnlineSuddenDeathChk","roundsOnlineSuddenDeathChk","raceOnlineSuddenDeathChk","atcOnlineSuddenDeathChk"];
for (const cid of sdIds) {
  const el = qs(cid);
  if (el) el.checked = !!suddenDeath;
}

// Mutual control toggles (online)
const mutualIds = ["arcadeOnlineMutualChk","hsOnlineMutualChk","roundsOnlineMutualChk","raceOnlineMutualChk","atcOnlineMutualChk"];
for (const mid of mutualIds) {
  const el = qs(mid);
  if (el) el.checked = !!mutualControl;
}

      // Hide Bull Challenge fields when not relevant
      setNumberRowVisible("arcadeOnlineVisitsInput", mode === "bull_challenge");
      setToggleRowVisible("arcadeOnlineSuddenDeathChk", mode === "bull_challenge");

      // Ensure stepper/toggle UI reflects the current control values
      try { arcadeSyncAllSetupUIControls(); } catch (_) {}
...
// Common competitive settings (starter / sudden death)
const starterChoice = String(st?.match?.arcade?.starter || "p1");
const suddenDeath = st?.match?.arcade?.suddenDeath === true;

const starterIds = ["bullLocalStarterSelect","hsLocalStarterSelect","roundsLocalStarterSelect","raceLocalStarterSelect","atcLocalStarterSelect"];
for (const sid of starterIds) {
  const el = qs(sid);
  if (el) el.value = starterChoice;
}

const sdIds = ["arcadeLocalSuddenDeathChk","hsLocalSuddenDeathChk","roundsLocalSuddenDeathChk","raceLocalSuddenDeathChk","atcLocalSuddenDeathChk"];
for (const cid of sdIds) {
  const el = qs(cid);
  if (el) el.checked = !!suddenDeath;
}
      ...
      setToggleRowVisible("arcadeLocalSuddenDeathChk", mode === "bull_challenge");
